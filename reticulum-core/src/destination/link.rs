...
pub fn linkid_from_packet(packet: &Packet) -> LinkId {
...
pub struct Link<E: LinkEventSink> {
    id: LinkId,
...
impl<E: LinkEventSink> Link<E> {
    pub fn new(destination: DestinationDesc, event_sink: E) -> Self {
        Self {
    ...
    pub fn request(&mut self) -> Packet {
        ...
        self.id = linkid_from_packet(&packet);
    ...
    fn post_event(&self, event: LinkEvent) {
        self.event_sink.send(LinkEventData {
            id: self.id,
