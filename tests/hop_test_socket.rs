...
use reticulum::{
    ...
    iface::{unix_socket_client::UnixSocketClient, unix_socker_server::UnixSocketServer},
...
async fn build_transport_full(
    ...
    transport.iface_manager().lock().await.spawn(
        UnixSocketServer::new(server_socket_path, transport.iface_manager()),
    ...
    for &addr in client_addr {
        transport
            ...
            .lock()
            .await
            .spawn(UnixSocketClient::new(addr), UnixSocketClient::spawn);
    }
...
async fn calculate_hop_distance() {
    ...
    let mut transport_a = build_transport("a", "/tmp/rns_socket_1a", &[]).await;
    let mut transport_b = build_transport("b", "/tmp/rns_socket_1b", &["/tmp/rns_socket_1a"]).await;
    let mut transport_c =
        build_transport("c", "/tmp/rns_socket_1c", &["/tmp/rns_socket_1a", "/tmp/rns_socket_1b"]).await;
...
async fn direct_path_request_and_response() {
    ...
    let mut transport_a = build_transport("a", "/tmp/rns_socket_2a", &[]).await;
    let mut transport_b = build_transport("b", "/tmp/rns_socket_2b", &["/tmp/rns_socket_2a"]).await;
...
async fn remote_path_request_and_response() {
    ...
    let mut transport_a = build_transport("a", "/tmp/rns_socket_3a", &[]).await;
    ...
    let mut transport_c = build_transport("c", "/tmp/rns_socket_3c", &["/tmp/rns_socket_3b"]).await;
