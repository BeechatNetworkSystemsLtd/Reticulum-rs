...
use reticulum_core::destination::link::linkid_from_packet;
use reticulum_core::destination::link::Link;
use reticulum_core::destination::link::LinkEventData;
use reticulum_core::destination::link::LinkHandleResult;
use reticulum_core::destination::link::LinkId;
use reticulum_core::destination::link::LinkStatus;
use reticulum_core::destination::validate_destination_announce;
...
#[derive(Clone)]
pub struct BroadcastLinkEventSink(tokio::sync::broadcast::Sender<LinkEventData>);
...
pub type BroadcastLink = Link<BroadcastLinkEventSink>;
...
struct TransportHandler {
    ...
    out_links: HashMap<AddressHash, Arc<Mutex<BroadcastLink>>>,
    in_links: HashMap<AddressHash, Arc<Mutex<BroadcastLink>>>,
...
impl Transport {
    ...
    pub async fn find_out_link(&self, link_id: &AddressHash) -> Option<Arc<Mutex<BroadcastLink>>> {
        self.handler.lock().await.out_links.get(link_id).cloned()
    }

    pub async fn find_in_link(&self, link_id: &AddressHash) -> Option<Arc<Mutex<BroadcastLink>>> {
    ...
    pub async fn link(&self, destination: DestinationDesc) -> Arc<Mutex<BroadcastLink>> {
        ...
        let mut link = Link::new(
            destination,
            BroadcastLinkEventSink(self.link_out_event_tx.clone()),
        );
...
async fn handle_announce<'a>(packet: &Packet, mut handler: MutexGuard<'a, TransportHandler>) {
    if let Ok(result) = validate_destination_announce(packet) {
        ...
        .cloned()
    {
        ...
        match destination.handle_packet(packet) {
            DestinationHandleStatus::LinkProof => {
                let link_id = linkid_from_packet(packet);
                if !handler.in_links.contains_key(&link_id) {
                    ...
                    let link = Link::new_from_request(
                        ...
                        destination.desc,
                        BroadcastLinkEventSink(handler.link_in_event_tx.clone()),
                    );
